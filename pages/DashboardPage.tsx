...
const parseHash = (hash: string): { appId: string | null; deepLinkInfo: string | null } => {
    ...
    let decodedDeepLinkInfo = deepLinkInfo || null;
    if (decodedDeepLinkInfo) {
        try {
            decodedDeepLinkInfo = decodeURIComponent(decodedDeepLinkInfo);
        } catch (e) {
            console.error('Failed to decode deepLinkInfo:', e);
        }
    }

    return { appId: appId || null, deepLinkInfo: decodedDeepLinkInfo };
};
...
const DashboardPage: React.FC<DashboardPageProps> = (props) => {
    ...
    const [displayMode, setDisplayMode] = useLocalStorage<'desktop' | 'webapp'>('htwth_display_mode', 'desktop');
        ...
        const fetchAndSubscribe = async () => {
            ...
            const unsubscribe = subscribeToChatMessages((newMsg) => {
                setChatMessages(prev => {
                    // Check if already exists (avoids duplicates from optimistic update)
    ...
    }, []); // Removed allUsers dependency to avoid re-subscribing too often
    ...
    const appComponentMap = useMemo(() => {
        ...
        const blogSpecificProps = {
            ...commonProps,
            posts: blogPosts,
            onSavePost: (post: any) => onSavePost(post, 'blog'),
            onDeletePost: (postId: string) => onDeletePost(postId, 'blog'),
        };

        const writeupSpecificProps = {
            ...commonProps,
            posts: writeups,
            onSavePost: (post: any) => onSavePost(post, 'writeup'),
            onDeletePost: (postId: string) => onDeletePost(postId, 'writeup'),
        };
        ...
        for (const app of allAppDefinitions) {
            ...
            if (Component) {
                if (app.id === 'blog') {
                    components[app.id] = <Component {...blogSpecificProps} />;
                } else if (app.id === 'writeup') {
                    components[app.id] = <Component {...writeupSpecificProps} />;
                } else {
                    components[app.id] = <Component {...commonProps} />;
                }
            }
        }
        return components;
    }, [user, blogPosts, writeups, onSavePost, onDeletePost, onLikePost, onAddCommentToPost, onDeleteCommentFromPost, handleNavigate]);
