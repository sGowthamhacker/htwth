                ...
                <div className="p-4 sm:p-6 md:p-8 border-t border-slate-200 dark:border-slate-800">
                    <div className="prose prose-lg dark:prose-invert max-w-none break-words overflow-hidden">
                        {post.content ? (
                            ...
                            <p className="text-slate-400">Start typing in the editor to see your post take shape...</p>
                        )}
                    </div>
                </div>
            </article>
            ...
            {filteredPosts.length > 0 ? (
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                    {filteredPosts.map(post => (
                        <PostCard key={post.id} post={post} currentUser={user} onClick={() => onNavigateWithinApp(`blog/${post.id}`)} onUpdatePost={async (updatedPost) => { await onSavePost(updatedPost); }} />
                    ))}
                </div>
            ) : (
                            ...
                            {canModify && (
                                <div className="relative" ref={optionsMenuRef}>
                                    <button 
                                        ...
                                        className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold text-sm hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
                                    >
                                    ...
                                    {isOptionsMenuOpen && (
                                        <div className="absolute right-0 top-full mt-2 w-40 bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700 z-20 animate-fade-in py-1">
                                            <button onClick={() => { openEditor(selectedPost); setOptionsMenuOpen(false); }} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 font-medium">
                                                <PencilIcon className="w-4 h-4" /> Edit
                                            </button>
                                            <button onClick={() => { handleDeletePost(selectedPost.id); setOptionsMenuOpen(false); }} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-500 hover:bg-slate-100 dark:hover:bg-slate-700 font-medium">
                            ...
                            <header className="p-4 sm:p-6 md:p-8">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    {selectedPost.tags.map((tag, index) => (
                                        <span key={`${tag}-${index}`} className={`px-3 py-1 rounded-full text-xs font-medium ${tagColorClasses[index % tagColorClasses.length]}`}>#{tag}</span>
                                    ))}
                                </div>
                            </header>
                            
                            <div className="p-6 sm:p-10 md:p-12 border-t border-slate-200 dark:border-slate-800">
                                <div className="prose prose-lg dark:prose-invert max-w-3xl mx-auto break-words overflow-hidden mb-10">
                                    <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]}>{selectedPost.content}</ReactMarkdown>
                                </div>
                                
                                {/* Social Share Section */}
                                <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-100 dark:bg-slate-800/40 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/50 gap-4 mb-4 shadow-sm animate-fade-in">
                                    <SocialShareButtons 
                                        title={selectedPost.title} 
                                        url={`${window.location.origin}${window.location.pathname}#blog/${selectedPost.id}`} 
                                    />
                                    <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
                                        <span>{new Date(selectedPost.created_at).toLocaleDateString()}</span>
                                        <span>•</span>
                                        <span>{Math.ceil(selectedPost.content.length / 500)} min read</span>
                                    </div>
                                </div>
                            </div>
