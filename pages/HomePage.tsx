...
const HomePage: React.FC<HomePageProps> = ({ user, writeups, blogPosts, isPending = false, onOpenApp = (appId, props, e) => {} }) => {
      ...
      <div className="space-y-10 p-4 sm:p-6 md:p-8 pb-32 relative z-10">
        ...
        <section>
            <div className="flex flex-wrap items-center justify-between mb-6 gap-2">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Recent Blog Posts</h2>
              <button onClick={(e) => onOpenApp('blog', {}, e)} className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 transition-colors">
                  View All &rarr;
              </button>
            </div>
            {/* Improved grid spacing and card container */}
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {blogPosts.slice(0, 3).map((post) => (
                <div key={post.id} className="transition-transform duration-300 hover:-translate-y-2">
                    <PostCard post={post} currentUser={user} onClick={(e) => onOpenApp('blog', { deepLinkInfo: post.id }, e)} />
                </div>
              ))}
            </div>
        </section>
