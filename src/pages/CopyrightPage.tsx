...
const CopyrightPage: React.FC<CopyrightPageProps> = ({ onClose }) => {
  ...
  const sections = [
    ...
    { id: 'sitemap', title: '15. Sitemap', color: 'text-emerald-600 dark:text-emerald-400' },
  ];
        ...
        `}>
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 hidden md:block">
                <div className="flex items-center justify-between mb-1">
                    ...
                    {onClose && (
                        <button onClick={onClose} className="p-2 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors">
                            <XCircleIcon className="w-6 h-6" />
                        </button>
                    )}
                </div>
        ...
        <main className="flex-1 flex flex-col h-full min-w-0 relative overflow-hidden md:bg-white md:dark:bg-slate-950">
            ...
            >
                <div className="max-w-4xl mx-auto p-5 sm:p-10 md:p-16 pb-24 md:pb-32">
                    ...
                    {/* Header */}
                    <header className="mb-10 md:mb-16 border-b border-slate-200 dark:border-slate-800 pb-8 relative">
                        {onClose && (
                            <button 
                                onClick={onClose} 
                                className="absolute -top-2 -right-2 md:top-0 md:right-0 p-2 text-slate-400 hover:text-slate-800 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 rounded-full transition-all md:hidden"
                                aria-label="Close"
                            >
                                <XCircleIcon className="w-6 h-6" />
                            </button>
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
                            ...
                            <div className="flex items-center gap-3 self-start sm:self-auto">
                                <button 
                                    onClick={handlePrint}
                                    className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-colors bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg"
                                >
                                ...
                                {onClose && (
                                    <button 
                                        onClick={onClose} 
                                        className="hidden md:flex items-center gap-2 text-sm font-semibold text-rose-500 hover:text-rose-700 dark:hover:text-rose-400 transition-colors bg-rose-50 dark:bg-rose-900/20 px-3 py-1.5 rounded-lg border border-rose-100 dark:border-rose-900/50"
                                    >
                    ...
                    <div className="space-y-16">
                        ...
                        {/* 15. Sitemap */}
                        <section id="sitemap" className="scroll-mt-24 pb-20">
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                                    <span className="text-emerald-600 dark:text-emerald-400 opacity-50">15.</span>
                                    Sitemap
                                </h2>
                                <button 
                                    onClick={() => {
                                        ...
                                        if (newWindow) {
                                          newWindow.document.write(`
                                            <html>
                                              <head>
                                                <title>HTWTH Sitemap JSON</title>
                                                <style>
                                    ...
                                    className="px-3 py-1.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-lg text-sm font-semibold hover:bg-emerald-200 dark:hover:bg-emerald-900/50 transition-colors"
                                >
                                    View JSON
                                </button>
                            </div>
                            <p className="text-slate-600 dark:text-slate-400 leading-relaxed mb-6">
                                Below is a complete overview of the public and authenticated areas of the HackToWriteToHack platform.
                            </p>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {/* Public Pages */}
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-xl border border-slate-100 dark:border-slate-800">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">Public Pages</h3>
                                    <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Home / Landing Page</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Features Overview</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Community Hub</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Resource Center</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Pricing Plans</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> ResumeAI (Coming Soon)</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Authentication (Login/Register)</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Legal & Copyright Center</li>
                                    </ul>
                                </div>

                                {/* Authenticated OS / Dashboard */}
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-xl border border-slate-100 dark:border-slate-800">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">Platform OS (Authenticated)</h3>
                                    <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Desktop Environment</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Writeup Editor (Markdown/AI)</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Terminal Emulator</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Global Chat Room</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Tools Directory</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Settings & Preferences</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> User Profile & Portfolio</li>
                                        <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Security Settings (2FA)</li>
                                    </ul>
                                </div>
                            </div>
                        </section>
