...
async function run() {
    console.log("Attempting anon update...");
    const postId = "749f1f81-b4d3-42f1-a4a8-f1812bc9619d";
    
    // Attempt update as anonymous using anon key
    const upd = await supabase.from('posts').update({ liked_by: ["123e4567-e89b-12d3-a456-426614174000", "local-test-anon"] }).eq('id', postId);
    console.log("Update result:", upd.error || upd.data);
