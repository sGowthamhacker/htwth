  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (newEventPayload && newEventPayload.ticket && newEventPayload.type) {
        ...
        if (smtpUser && smtpPass && ticket?.userEmail) {
          try {
            ...
            const messageText = message || (Array.isArray(ticket.messages) && ticket.messages.length > 0 ? ticket.messages[0].message : '');
            ...
            console.error("Failed to send support ticket email:", err?.message || err);
          }
        }
      }
    ...
    console.error("Error in /api/support/tickets:", err?.message || err);
