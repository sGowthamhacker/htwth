...
const getSupabaseCredentials = () => {
    // Prioritize environment variables, but ensure we don't use the old/wrong URL
    let url = import.meta.env.VITE_SUPABASE_URL || '';
    let anonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || '';

    const OLD_URL = 'https://jlxbccknnkkazvivgihq.supabase.co';
    const NEW_URL = 'https://greuvmplqatnvbuymnof.supabase.co';
    const NEW_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdyZXV2bXBscWF0bnZidXltbm9mIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE4NTYwNzMsImV4cCI6MjA3NzQzMjA3M30.O_J3NEneEBZ1PRN_EX1ik1cfb472cnX5_q8DgP16T8g';

    // If URL is missing, placeholder, or the old one, use the new one
    if (!url || url.includes('example.com') || url.trim().replace(/\/$/, '') === OLD_URL) {
        url = NEW_URL;
        anonKey = NEW_KEY;
    }

    // Double check anonKey
...
let inMemoryContactRequests: ContactRequest[] = [];
let inMemoryBounties: any[] = [];
...
const USER_COLUMNS = 'id, name, email, avatar, role, writeup_access, has_requested_writeup_access, bio, gender, skills, is_profile_private, friends, friend_requests, status, created_at, verified_at, admin_verified, doc_access, firebase_uid, welcome_email_sent, is_2fa_enabled, totp_secret, backup_codes, wallpaper, desktop_preferences';
const POST_COLUMNS = 'id, type, title, content, created_at, comments, tags, severity, liked_by, reward, author_id, is_visible';
...
/**
 * Returns the correct base path for backend functions depending on the hosting platform.
 */
export const getApiPath = (functionName: string): string => {
    const isNetlify = window.location.hostname.includes('netlify.app');
    const isVercel = window.location.hostname.includes('vercel.app');
    
    if (isNetlify) return `/.netlify/functions/${functionName}`;
    if (isVercel) return `/api/${functionName}`;
    
    // Default fallback for preview environment or local dev
...
// --- Bounty Functions ---
export const getBounties = async (): Promise<any[]> => { return safeDbQuery(async () => { const { data, error } = await supabase!.from('bounties').select('*').order('createdAt', { ascending: false }); if (error) throw error; return { data, error: null }; }, [...inMemoryBounties], "getBounties"); };
export const addBounty = async (bounty: any): Promise<any> => { const newBounty = { ...bounty, id: bounty.id || crypto.randomUUID() }; inMemoryBounties.unshift(newBounty); return safeDbQuery(async () => { const { data, error } = await supabase!.from('bounties').insert(newBounty).select('*').single(); if (error) throw error; return { data, error: null }; }, newBounty, "addBounty"); };
export const deleteBountyDb = async (id: string): Promise<boolean> => { inMemoryBounties = inMemoryBounties.filter(b => b.id !== id); return safeDbQuery(async () => { const { error } = await supabase!.from('bounties').delete().eq('id', id); if (error) throw error; return { data: true, error: null }; }, true, "deleteBountyDb"); };
export const updateBounty = async (id: string, updates: any): Promise<any> => { const idx = inMemoryBounties.findIndex(b => b.id === id); if (idx > -1) inMemoryBounties[idx] = { ...inMemoryBounties[idx], ...updates }; const fallback = idx > -1 ? inMemoryBounties[idx] : null; return safeDbQuery(async () => { const { data, error } = await supabase!.from('bounties').update(updates).eq('id', id).select('*').single(); if (error) throw error; return { data, error: null }; }, fallback, "updateBounty"); };
...
export const getContactRequests = async (): Promise<ContactRequest[]> => {
    const fallback = [...inMemoryContactRequests].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    return safeDbQuery(
        async () => {
            ...
            if (error) throw error;
            return { data: data as ContactRequest[], error: null };
        },
        fallback,
        "getContactRequests"
    );
...
export const addContactRequest = async (request: Omit<ContactRequest, 'id' | 'created_at' | 'handled'>): Promise<ContactRequest | null> => {
    ...
    inMemoryContactRequests.unshift(newRequest);
...
export const updateContactRequestStatus = async (id: string, handled: boolean): Promise<boolean> => {
    const idx = inMemoryContactRequests.findIndex(r => r.id === id);
    if (idx > -1) inMemoryContactRequests[idx].handled = handled;
...
export const addCommentToPost = async (postId: string, post: Post, newComment: Comment): Promise<Post | null> => { 
    // Prepare the fallback optimistic update
    const list = inMemoryWriteups.find(p => p.id === postId) ? inMemoryWriteups : inMemoryBlogs;
    const idx = list.findIndex(p => p.id === postId);
    if (idx > -1) { list[idx] = { ...list[idx], comments: [newComment, ...(list[idx].comments || [])] }; }
    const fallback = idx > -1 ? list[idx] : { ...post, comments: [newComment, ...(post.comments || [])] };

    return safeDbQuery(async () => {
        // 1. Fetch latest post
        const { data: latestPost, error: fetchError } = await supabase!.from('posts').select('comments, type').eq('id', postId).single();
        if (fetchError) throw fetchError;
        
        // 2. Append comment
        const currentComments = latestPost?.comments || [];
        const updatedComments = [...currentComments, newComment]; // Add to end or beginning; let's say end, or keep original append
        
        // 3. Update
        const { data, error } = await supabase!.from('posts').update({ comments: updatedComments }).eq('id', postId).select(POST_COLUMNS).single();
        if (error) throw error;
        
        // need to sanitize
        let finalAuthor = post.author;
        if (data.author_id) {
             const { data: userData } = await supabase!.from('users').select(USER_COLUMNS).eq('id', data.author_id).single();
             if (userData) finalAuthor = sanitizeUser(userData);
        }
        return { data: sanitizePost({ ...data, author: finalAuthor }), error: null };
    }, fallback, "addCommentToPost");
};

export const likePost = async (postId: string, post: Post, userId: string): Promise<Post | null> => { 
    // Prepare fallback
    const list = inMemoryWriteups.find(p => p.id === postId) ? inMemoryWriteups : inMemoryBlogs;
    const idx = list.findIndex(p => p.id === postId);
    const newLikedBy = post.liked_by?.includes(userId) ? post.liked_by.filter(id => id !== userId) : [...(post.liked_by || []), userId];
    if (idx > -1) { list[idx] = { ...list[idx], liked_by: newLikedBy }; }
    const fallback = idx > -1 ? list[idx] : { ...post, liked_by: newLikedBy };

    return safeDbQuery(async () => {
        const { data: latestPost, error: fetchError } = await supabase!.from('posts').select('liked_by, type').eq('id', postId).single();
        if (fetchError) throw fetchError;

        const currentLikes = latestPost?.liked_by || [];
        const updatedLikes = currentLikes.includes(userId) ? currentLikes.filter((id: string) => id !== userId) : [...currentLikes, userId];

        const { data, error } = await supabase!.from('posts').update({ liked_by: updatedLikes }).eq('id', postId).select(POST_COLUMNS).single();
        if (error) throw error;
...
export const subscribeToChatMessages = (onNewMessage: (msg: ChatMessage) => void) => {
    if (!supabase || hasFallenBackToMock) return () => {};
    ...
    const channel = supabase
        ...
        .on(
            ...
            async (payload) => {
                ...
                const { data: userData } = await supabase!.from('users').select(USER_COLUMNS).eq('id', newMsg.author_id).single();
                const msgWithAuthor: ChatMessage = {
                    ...newMsg,
                    author: userData ? sanitizeUser(userData) : { name: 'Unknown', avatar: '' }
                };
                onNewMessage(msgWithAuthor);
            }
        )
        .subscribe();
